...
def _loaders(dataset, metadata, model_config, settings, seed, mode, ledger=None, rho=0.0, dry_run=False):
    ...
    kwargs = {"batch_size": int(model_config["training"]["batch_size"]), "num_workers": int(model_config["training"]["num_workers"])}
    return (
...
def _train_and_audit(dataset, metadata, model_config, audit_config, settings, output, seed, mode, triplets, device, dry_run, ledger=None, rho=0.0):
    import torch
    train_loader, val_loader, id_loader, transform = _loaders(dataset, metadata, model_config, settings, seed, mode, ledger, rho, dry_run)
    ...
    _, id_predictions = evaluate(model, id_loader, device, True, "control ID validation")
    ...
    predictions, qa = infer_triplets(model, dataset, selected, transform, device=device, roi_size=32, feather_width=4, show_progress=True)
...
def main() -> None:
    ...
    if experiment == "context_randomized_control":
        ...
        for rho in map(float, settings["strengths"]):
            ...
            completed.append({"rho": rho, "status": "completed"})
