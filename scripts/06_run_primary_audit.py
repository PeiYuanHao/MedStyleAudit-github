...
def main() -> None:
    ...
    config["checkpoint"] = str(args.checkpoint); run = start_run("primary_audit", config, output, seed, overwrite=args.overwrite)
    ...
    aggregate_audit(predictions, id_table[logit_column], output, float(config["audit"]["q_min"]), int(config["audit"].get("bootstrap_draws", 0)), seed)
