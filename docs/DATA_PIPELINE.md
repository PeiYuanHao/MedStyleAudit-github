...
`descriptors.parquet` contains canonical metadata (`source_id`, `split`, hospital,
...
`triplets.parquet` has one row per donor pair and retains source/within/cross
physical identities, distances, cost, and donor multiplicity index. Removing a
construction-invalid pair removes both arms. `source_ledger.parquet` retains every
candidate directed comparison and its exclusion reason.

`audit_records.parquet` stores original, within, and cross logits on the same row.
